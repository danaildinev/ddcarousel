...
describe("Pagination module", () => {
    ...
    it("creates pagination and changes pages when dots are clicked", async () => {
        ...
        expect(pagination()[0]?.hasAttribute(activeDataAttr)).toBe(true);

        pagination()[1]?.click();

        expect(carousel.getCurrentPage()).toBe(1);
        expect(pagination()[1]?.hasAttribute(activeDataAttr)).toBe(true);
    });
    ...
    it("marks only the first dot as active after initialization", async () => {
        ...
        const active = pagination().filter(dot => dot.hasAttribute(activeDataAttr));
    ...
    it("removes active class from previous dot when page changes", async () => {
        ...
        expect(pagination()[0]?.hasAttribute(activeDataAttr)).toBe(false);
        expect(pagination()[1]?.hasAttribute(activeDataAttr)).toBe(false);
        expect(pagination()[2]?.hasAttribute(activeDataAttr)).toBe(true);
        expect(pagination().filter(dot => dot.hasAttribute(activeDataAttr))).toHaveLength(1);
    });
    ...
    it("updates active dot across multiple page changes", async () => {
        ...
        pagination()[3]!.click();
        expect(pagination()[1]?.hasAttribute(activeDataAttr)).toBe(false);
        expect(pagination()[3]?.hasAttribute(activeDataAttr)).toBe(true);

        pagination()[0]!.click();
        expect(pagination()[3]?.hasAttribute(activeDataAttr)).toBe(false);
        expect(pagination()[0]?.hasAttribute(activeDataAttr)).toBe(true);
        expect(pagination().filter(dot => dot.hasAttribute(activeDataAttr))).toHaveLength(1);
    });
